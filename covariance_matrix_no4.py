...
def full_error_vec(er_2e,er_3e, er_2u, er_3u): #makes the giant vector with all the systematic and luminosity errors on.
    A = np.concatenate((er_2e,er_3e), axis=1)
    C = np.concatenate((A,er_2u), axis=1)
    D = np.concatenate((C,er_3u), axis=1)
    return D


def full_cross_vec(xsec_2e,xsec_3e,xsec_2u,xsec_3u): #makes the full vector with all the channels and jets in.
    A = np.append(xsec_2e,xsec_3e)
    C = np.append(A, xsec_2u)
...
def Cov_syst100(e, y):
    ...
    for i in np.arange(len(e[0,:])):
        for j in np.arange(len(e[0,:])):
            print i,j
            X[i,j] = ((Cov_syst2(e)[i,j])/100) * ((y[i]*y[j]))
    return X
...
if __name__ =="__main__": 
    ...
    Cov_mega['one type test'] = Cov_syst100(full_error_vec(er_2e,er_3e, er_2u, er_3u),full_stat_vec(st_e2,st_e3,st_u2,st_u3))
    ...
    #Cov_mega['FULL_syst+lumi_no4'] = Cov_syst100(full_error_vec(er_2e,er_3e, er_2u, er_3u),full_cross_vec(xsec_2e,xsec_3e,xsec_2u,xsec_3u))
    #Cov_mega['FULL_ALLTHREE_no4'] = Cov_full(full_stat_vec(st_e2,st_e3,st_u2,st_u3),Cov_mega['FULL_syst+lumi_no4'])
